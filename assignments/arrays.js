...
// dealer find out which car has an id of 33 by logging the car's year, make, and model in the 
// console log provided to you below:
function getCar(arr, id) {
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].id === id) {
...
// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model
//  of the last car in the inventory?  Log the make and model into the console.
function lastCar(arr) {
    for (let i = 0; i < arr.length; i++) {
        if (i === arr.length - 1) {
        console.log(`The last car in the current inventory is a ${arr[i].car_make} ${arr[i].car_model}` )
        }
    }
}
...
// carYears array you just created, find out how many cars were made before the year 2000 by
//  populating the array oldCars and logging it's length.
...
// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array
//  that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() 
// to show the results of the array in the console.
function getGermans(arr) {
